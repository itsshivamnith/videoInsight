...
const STORE_KEYS = (videoId) => ({
  ...
  segments: `ls_segments_${videoId}`,
  summary: `ls_summary_${videoId}`,
  quiz: `ls_quiz_${videoId}`,
  flashcards: `ls_flashcards_${videoId}`,
...
const Player = () => {
  ...
  useEffect(() => {
    ...
    setSegments(storageGet(keys.segments) || []);
    setSummary(storageGet(keys.summary) || "");
    setQuiz(Array.isArray(storageGet(keys.quiz)) ? storageGet(keys.quiz) : []);
    setFlashcards(Array.isArray(storageGet(keys.flashcards)) ? storageGet(keys.flashcards) : []);
  }, [activeVideoId]);
  ...
  const fetchTranscriptForActive = useCallback(
    async (opts = {}) => {
      ...
      try {
        ...
        if (segmentsData.length > 0) storageSet(keys.segments, segmentsData); // persist
        setErr("");
      } catch (e) {
  ...
  const handleFlashcards = useCallback(async () => {
    ...
    if (!summary && !transcript) {
      setErr("Please generate summary or transcript first to create flashcards.");
      return;
    }
    setViewMode("flashcards");

    const keys = STORE_KEYS(activeVideoId);
    const savedCards = storageGet(keys.flashcards);
    ...
    setFlashcards([]);
    try {
      const res = await fetch(`${BASE_URL}/api/ai/flashcards`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      storageSet(keys.flashcards, data.flashcards);
    } catch (e) {
      setErr(e.message || "Failed to generate flashcards");
    } finally {
      setFlashcardsLoading(false);
    }
  }, [activeVideoId, isAuthenticated, startGoogleSignIn, summary, transcript]);

  // Auto-generate flashcards when flashcards viewMode is selected
  useEffect(() => {
    if (viewMode === "flashcards" && flashcards.length === 0 && !flashcardsLoading) {
      handleFlashcards();
    }
  }, [viewMode, flashcards.length, flashcardsLoading, handleFlashcards]);
  ...
  const handleTranslateToEnglish = useCallback(async () => {
    ...
    try {
      ...
      setTranscript(data.translated);
      setSegments([]); // Clear out segments since we display plain text now
      storageSet(STORE_KEYS(activeVideoId).transcript, data.translated);
      storageSet(STORE_KEYS(activeVideoId).segments, []);
    } catch (e) {
      setErr(e.message || "Failed to translate transcript");
    } finally {
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-[calc(100vh-64px)] bg-primary overflow-y-auto lg:overflow-hidden relative">
      ...
      <AnimatePresence>
        {playlistVideos.length > 0 && isPlaylistOpen && (
          <motion.div
            ...
            className="absolute left-0 top-0 bottom-0 glass backdrop-blur-md border-r border-theme/50 z-[60] flex flex-col shadow-2xl rounded-r-3xl overflow-hidden"
      ...
      {/* Right: tools */}
      <div className="flex-1 w-full lg:flex-none lg:w-[400px] xl:w-[450px] bg-surface shadow-xl border-l border-theme flex flex-col z-20 overflow-visible lg:overflow-hidden">
        {/* Header / Controls */}
